...
func handleAllocateRequest(r Request, m *stun.Message) error {
	...
	if !hasAuth {
		r.Log.Warnf("Failed to authenticate request: %v", err)
		return err
	}
	...
	r.Log.Debugf("FiveTuple Created. SrcAddr: %s, DstAddr: %s, Protocol: %d", fiveTuple.SrcAddr, fiveTuple.DstAddr, fiveTuple.Protocol)
	requestedPort := 0
	reservationToken := ""
	...
	if alloc := r.AllocationManager.GetAllocation(fiveTuple); alloc != nil {
		...
		if id != m.TransactionID {
			r.Log.Warnf("Allocation mismatch for %v", fiveTuple)
		...
		// A retry allocation
		r.Log.Debugf("Retry allocation for %v", fiveTuple)
	...
	if err = requestedTransport.GetFrom(m); err != nil {
		r.Log.Warnf("Failed to get RequestedTransport: %v", err)
		...
		r.Log.Warnf("Unsupported transport protocol: %v", requestedTransport.Protocol)
	...
	if m.Contains(stun.AttrDontFragment) {
		r.Log.Warn("DONT-FRAGMENT attribute is not supported")
	...
	if err = reservationTokenAttr.GetFrom(m); err == nil {
		...
		if err = evenPort.GetFrom(m); err == nil {
			r.Log.Warn("Request must not contain RESERVATION-TOKEN and EVEN-PORT")
	...
	if err = evenPort.GetFrom(m); err == nil {
		...
		if err != nil {
			r.Log.Warn("Insufficient capacity for even port")
		...
		if err != nil {
			r.Log.Warn("Failed to generate reservation token")
			return err
		}
	}
	...
	a, err := r.AllocationManager.CreateAllocation(
		...
		lifetimeDuration)
	if err != nil {
		r.Log.Warn("Insufficient capacity for allocation")
		return buildAndSendErr(r.Conn, r.SrcAddr, err, insufficientCapacityMsg...)
	}
	...
	if err != nil {
		r.Log.Warn("Failed to get source IP and port")
		return buildAndSendErr(r.Conn, r.SrcAddr, err, badRequestMsg...)
	}

	relayIP, relayPort, err := ipnet.AddrIPPort(a.RelayAddr)
	if err != nil {
		r.Log.Warn("Failed to get relay IP and port")
		return buildAndSendErr(r.Conn, r.SrcAddr, err, badRequestMsg...)
	}
	...
	if reservationToken != "" {
		r.Log.Debugf("Reservation token: %s", reservationToken)
	...
	r.Log.Debugf("Sending success response for %v", fiveTuple)
...
func handleRefreshRequest(r Request, m *stun.Message) error {
	...
	if lifetimeDuration != 0 {
		...
		if a == nil {
			r.Log.Warnf("No allocation found for %v", fiveTuple)
			return fmt.Errorf("%w %v:%v", errNoAllocationFound, r.SrcAddr, r.Conn.LocalAddr())
		}
		r.Log.Debugf("Refreshing allocation for %v", fiveTuple)
		a.Refresh(lifetimeDuration)
	} else {
		r.Log.Debugf("Deleting allocation for %v", fiveTuple)
...
func handleSendIndication(r Request, m *stun.Message) error {
	...
	if perm := a.GetPermission(msgDst); perm == nil {
		r.Log.Infof("permission denied for client %s to peer %s", r.SrcAddr, msgDst)
...
func handleChannelBindRequest(r Request, m *stun.Message) error {
	...
	if a == nil {
		r.Log.Warnf("No allocation found for %v", r.SrcAddr)
		return fmt.Errorf("%w %v:%v", errNoAllocationFound, r.SrcAddr, r.Conn.LocalAddr())
	}
	...
	if err = channel.GetFrom(m); err != nil {
		r.Log.Warnf("Failed to get ChannelNumber: %v", err)
		return buildAndSendErr(r.Conn, r.SrcAddr, err, badRequestMsg...)
	}

	peerAddr := proto.PeerAddress{}
	if err = peerAddr.GetFrom(m); err != nil {
		r.Log.Warnf("Failed to get PeerAddress: %v", err)
		return buildAndSendErr(r.Conn, r.SrcAddr, err, badRequestMsg...)
	}
	...
	if err != nil {
		r.Log.Warnf("Failed to bind channel %d to %s: %v", channel, peerAddr, err)
		return buildAndSendErr(r.Conn, r.SrcAddr, err, badRequestMsg...)
	}

	r.Log.Debugf("Channel %d bound to %s", channel, peerAddr)
